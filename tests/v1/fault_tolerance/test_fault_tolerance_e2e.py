...
@pytest.mark.skipif(not has_nixl_ep(), reason="Requires nixl_ep all2all backend")
@multi_gpu_test(num_gpus=4)
def test_consecutive_scale_downs_use_current_coordinates():
    """Two consecutive scale_downs keep recovering correctly.

    Regression test: the second victim's current (densified) rank differs from
    its original rank. removed_dp_ranks must come from the survivor's status
    ``dead_dp_ranks`` (current coordinates) verbatim; earlier-round masks must
    also survive, or a stale-rank all2all timeout causes a phantom fault.
    """
    eplb_args = [
        "--enable-eplb",
        "--eplb-config.num_redundant_experts",
    ...
    with _ft_manager(eplb_args, dp_size=dp_size) as servers:
        assert len(servers) == dp_size
        servers_by_rank = _servers_by_rank(servers, dp_size)
        _assert_serving_and_healthy(list(servers))

        alive = [0, 1, 2, 3]  # original ranks still in the cluster
        for victim_rank in (1, 3):
            _kill_worker_process(servers_by_rank[victim_rank])
            active = {r: servers_by_rank[r] for r in alive}
            faulted = _drive_to_faulted(active, match_values={"dead", "unhealthy"})
            assert faulted[victim_rank]["status"] == "dead", faulted[victim_rank]

            # The orchestrator is stateless: read the dead ranks (current
            # coordinates) from a survivor's status and send them back.
            reporter = next(r for r in alive if r != victim_rank)
            removed = faulted[reporter].get("dead_dp_ranks")
            assert removed == [alive.index(victim_rank)], faulted[reporter]

            alive.remove(victim_rank)
            survivors = [servers_by_rank[r] for r in alive]
            for server in survivors:
                _apply_ft(server, "scale_down", {"removed_dp_ranks": removed})
            _assert_serving_and_healthy(survivors)
