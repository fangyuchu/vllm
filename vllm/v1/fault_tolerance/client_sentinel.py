...
class ClientSentinel(BaseSentinel):
    def __init__(
        ...
        # todo: create the task for fault handling
    ...
    # todo: implement retry and pause with masked kernel
    ...
    async def run(self):
        ...
        try:
            while not self.sentinel_dead:
                ...
                # todo: check if shutdown is still needed here
                if not self.is_faulted.is_set():
                    self.is_faulted.set()
                    # todo: check the timeout settings
    ...
    async def poll_and_execute_cmds(self):
