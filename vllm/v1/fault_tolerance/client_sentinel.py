...
class RetryParams(TypedDict, total=False):
    new_stateless_dp_group_port: NotRequired[int]
    timeout: float | int
...
class ClientSentinel(BaseSentinel):
    ...
    # todo: change the parameter to ft_request
    async def retry(self, ft_request: FaultToleranceRequest) -> FaultToleranceResult:
        """Expected params: timeout, new_stateless_dp_group_port (optional)."""
        params = cast(RetryParams, ft_request.params)
        if "new_stateless_dp_group_port" not in params:
            params["new_stateless_dp_group_port"] = get_open_port()

        # Retry is only meaningful if all managed engines are still alive.
        for engine_status in self.engine_status_dict.values():
            if engine_status["status"] == "dead":
                return FaultToleranceResult(
                    request_id=ft_request.request_id,
                    success=False,
                    reason="dead engines present",
                )

        result = await self._execute_cmd_on_engines(ft_request, self.engine_identities)

        if result.success:
            self.is_faulted.clear()
            # todo: consider let engine report healthy state so status can be updated by
            # fault_receiver_socket.
            for i in self.engine_status_dict:
                self.engine_status_dict[i] = {"status": "healthy"}
            await self._pub_engine_status()
        return result

    async def pause(self, ft_request: FaultToleranceRequest) -> FaultToleranceResult:
