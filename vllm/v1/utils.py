...
def wait_for_completion_or_failure(
    ...
    try:
        ...
        if engine_manager:
            engine_dead_flag = {"dead": False, "reason": None}

            def engine_down_callback(dead_proc, all_processes: list):
                engine_dead_flag["dead"] = True
                dead_engine_index = all_processes.index(dead_proc)
                engine_dead_flag["reason"] = (
                    f"Engine process {dead_engine_index} is dead."
                )
        ...
        while sentinel_to_proc:
            if engine_manager and engine_dead_flag["dead"]:
                raise RuntimeError(engine_dead_flag["reason"])
