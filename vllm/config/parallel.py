...
class ParallelConfig:
    ...
    def compute_hash(self):
        ...
        # num_redundant_experts is derived at runtime during elastic EP
        # scale-up and can legitimately differ between a front-end process and
        # the worker process it is handshaking with before runtime state sync
        # completes. Exclude only this derived field while still keeping the
        # rest of EPLBConfig (e.g. enable flags / logging / async mode) in the
        # hash so we continue to catch real collective-communication mismatches.
        hashable_parallel_config = replace(
            self,
            eplb_config=replace(self.eplb_config, num_redundant_experts=0),
        )
        factors = get_hash_factors(hashable_parallel_config, ignored_factors)
